...
-- Función para actualizar datos de un administrado
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE NOTICE 'Violación de restricción UNIQUE. adm_numdocumento ya existe: %', SQLERRM;
        RETURN -1;
    WHEN not_null_violation THEN
        RAISE NOTICE 'Violación de restricción NOT NULL: %', SQLERRM;
        RETURN -2;
...
-- Función para eliminar un administrado
CREATE OR REPLACE FUNCTION usuariocasilla_eliminar(p_adm_id BIGINT) RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    rows_affected INTEGER;
BEGIN
    DELETE FROM siga_administrado AS sa WHERE sa.adm_id = p_adm_id;
    GET DIAGNOSTICS rows_affected = ROW_COUNT;
    RETURN rows_affected;
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE NOTICE 'No se puede eliminar el administrado porque hay registros dependientes: %', SQLERRM;
        RETURN -1;
    WHEN OTHERS THEN
